...
const SIZE: (u32, u32) = (800 / DISPLAY_FACTOR, 600 / DISPLAY_FACTOR);
...
fn main() {
    App::new()
        ...
        .add_systems(FixedUpdate, switch_textures)
        .run();
}

#[derive(Resource, Clone, ExtractResource)]
pub struct GameOfLifeBuffers {
    pub buffer_color: Buffer,
    pub buffer_alive: Buffer,
}
...
fn setup(mut commands: Commands, mut images: ResMut<Assets<Image>>, device: Res<RenderDevice>) {
    ...
    let buffer_color = utils::create_storage_buffer_with_data(
        &device,
        &[[0., 0., 0., 1.]],
        Some("Color storage buffer"),
    );

    let buffer_alive =
        utils::create_storage_buffer_with_data(&device, &[0], Some("Color storage buffer"));
...
fn prepare_bind_group(
    ...
    let bind_group_0 = render_device.create_bind_group(
        ...
        &BindGroupEntries::sequential((
            &view_a.texture_view,
            &view_b.texture_view,
            buffers.buffer_color.as_entire_binding(),
            buffers.buffer_alive.as_entire_binding(),
        )),
    );
